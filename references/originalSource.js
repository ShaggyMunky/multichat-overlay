...
const groupConsecutiveMessages = GetBooleanParam(
...
const showTwitchAnnouncements = GetBooleanParam(
  "showTwitchAnnouncements",
  true
);
const showTwitchSubs = GetBooleanParam("showTwitchSubs", true);
const showTwitchChannelPointRedemptions = GetBooleanParam(
  "showTwitchChannelPointRedemptions",
  true
);
...
const showYouTubeSuperStickers = GetBooleanParam(
  "showYouTubeSuperStickers",
  true
);
const showYouTubeMemberships = GetBooleanParam("showYouTubeMemberships", true);

const showStreamlabsDonations = GetBooleanParam(
  "showStreamlabsDonations",
  true
);
...
const showTipeeeStreamDonations = GetBooleanParam(
  "showTipeeeStreamDonations",
  true
);
...
const ignoreUserList =
  ignoreChatters.split(",").map((item) => item.trim().toLowerCase()) || [];
...
async function TwitchChatMessage(data) {
  ...
  // Set furry mode
  if (furryMode) message = TranslateToFurry(message);
  ...
  if (data.message.isMe) messageDiv.style.color = messageColor;
  ...
  // Embed image
  if (
    IsThisUserAllowedToPostImagesOrNotReturnTrueIfTheyCanReturnFalseIfTheyCannot(
...
async function TwitchAnnouncement(data) {
  if (!showTwitchAnnouncements) return;
  ...
  for (i in data.parts) {
    if (data.parts[i].type == `emote`) {
      ...
      const regex = new RegExp(regexPattern, "g");
      content.querySelector("#message").innerHTML = content
        .querySelector("#message")
        .innerHTML.replace(regex, emoteElement);
    }
  }
...
async function TwitchSub(data) {
  ...
  if (!isPrime)
    titleDiv.innerText = `${username} subscribed with Tier ${subTier.charAt(
      0
    )}`;
  else titleDiv.innerText = `${username} used their Prime Sub`;
...
async function TwitchResub(data) {
  ...
  if (!isPrime)
    titleDiv.innerText = `${username} resubscribed with Tier ${subTier.charAt(
      0
...
async function TwitchGiftSub(data) {
  ...
  titleDiv.innerText = `${username} gifted a Tier ${subTier.charAt(
    0
...
async function TwitchRewardRedemption(data) {
  if (!showTwitchChannelPointRedemptions) return;
...
function YouTubeMessage(data) {
  if (!showYouTubeMessages) return;

  // Don't post messages starting with "!"
  if (data.message.startsWith("!") && excludeCommands) return;
  ...
  // Set furry mode
  if (furryMode) messageDiv.innerText = TranslateToFurry(data.message);
...
function YouTubeSuperSticker(data) {
  ...
  stickerInstance.querySelector(
    "#stickerLabel"
  ).innerText = `${data.user.name} sent a Super Sticker (${data.amount})`;
...
function PatreonPledgeCreated(data) {
  if (!showPatreonMemberships) return;
...
function KofiDonation(data) {
  ...
  else titleDiv.innerHTML = `${kofiIcon} ${user} donated ${currency} ${amount}`;

  if (message != null) contentDiv.innerHTML = `${message}`;
...
function TipeeeStreamDonation(data) {
  if (!showTipeeeStreamDonations) return;
...
function FourthwallOrderPlaced(data) {
  if (!showFourthwallAlerts) return;
  ...
  if (message.trim() != "") contentDiv.innerHTML = `${message}`;
...
function IsThisUserAllowedToPostImagesOrNotReturnTrueIfTheyCanReturnFalseIfTheyCannot(
  targetPermissions,
  data,
  platform
) {
